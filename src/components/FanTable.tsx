...
interface Props {
  ...
  onUpdateQuota: (quota: number | undefined) => void;
}
...
export default function FanTable({
  ...
  const enterQuota = () => {
    setQuotaInput(club.quota != null ? String(club.quota) : "");
    setQuotaMode(true);
    setBulkMode(false);
  };
  ...
  const saveBulk = () => {
    onBulkUpdate(club.members.map((m) => ({
      memberId: m.id,
    ...
    <>
      ...
      {!removeMode && (
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            {quota != null && !quotaMode && (
              <span className="text-[10px] font-mono text-amber/50 bg-amber/8 px-1.5 py-0.5 rounded-md">
            ...
            {quotaMode ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  onKeyDown={(e) => { if (e.key === "Enter") saveQuota(); if (e.key === "Escape") cancelQuota(); }}
                  placeholder="fan gain target…"
                  autoFocus
                  className="bg-surface border border-amber/20 rounded-lg px-3 py-1.5 text-xs font-mono text-amber/80 placeholder:text-white/15 focus:outline-none focus:border-amber/40 transition-colors w-40"
                />
                <button onClick={cancelQuota} className="px-2.5 py-1.5 rounded-lg border border-white/10 text-white/35 text-xs hover:border-white/20 transition-colors">✕</button>
                <button onClick={saveQuota} className="px-3 py-1.5 rounded-lg bg-amber/80 text-black text-xs font-semibold hover:bg-amber transition-colors">Set</button>
              </div>
            ) : bulkMode ? (
              <>
        ...
        <table className="w-full border-collapse" style={{ minWidth: 480 }}>
          ...
          <tbody>
            {club.members.map((member, idx) => {
              ...
              // Quota coloring: green if met, red if not (only when hasData)
              const meetsQuota = quota != null && hasData && increase >= quota;
              const missesQuota = quota != null && hasData && increase < quota;
              const increaseColor = meetsQuota
                ? "text-emerald"
                : missesQuota
                ? "text-rose"
                : increase > 0
              ...
              return (
                ...
                >
                  <td className="w-10 text-center align-middle py-3">
                    {removeMode && (
                      <div className={`mx-auto w-4 h-4 rounded flex items-center justify-center border transition-all ${isSelected ? "bg-rose border-rose" : "border-white/20"}`}>
                        {isSelected && (
                  ...
                  <td className="px-4 py-3 text-right w-[130px]">
                    {bulkMode ? (
                      <input ref={idx === 0 ? firstInputRef : undefined} type="number"
                        value={draft[member.id]?.prev ?? ""} onChange={(e) => updateDraft(member.id, "prev", e.target.value)}
                        placeholder="0"
                      ...
                      <span className="text-xs font-mono text-white/30">{prev > 0 ? formatFans(prev) : <span className="opacity-30">—</span>}</span>
                    )}
                  </td>

                  <td className="px-4 py-3 text-right w-[130px]">
                    {bulkMode ? (
                      <input type="number" value={draft[member.id]?.current ?? ""} onChange={(e) => updateDraft(member.id, "current", e.target.value)}
                        placeholder="0" onKeyDown={(e) => { if (e.key === "Enter" && idx === club.members.length - 1) saveBulk(); }}
                        className="w-full bg-surface-raised border border-white/10 rounded-lg px-2 py-1.5 text-xs font-mono text-white/80 text-right placeholder:text-white/15 focus:outline-none focus:border-white/25 transition-colors" />
                    ) : (
                      <span className="text-xs font-mono text-white/65">{curr > 0 ? formatFans(curr) : <span className="opacity-30">—</span>}</span>
                    )}
                  </td>

                  <td className="px-4 py-3 text-right w-[100px]">
                    {hasData ? (
                      <span className={`text-xs font-mono font-semibold whitespace-nowrap ${increaseColor}`}>
      ...
      {bulkMode && (
        <div className="mt-3 flex gap-2 animate-fade-in">
          <button onClick={cancelBulk} className="flex-1 py-2.5 rounded-xl border border-white/10 text-white/35 text-sm hover:border-white/20 transition-colors">Cancel</button>
          <button onClick={saveBulk} className="flex-1 py-2.5 rounded-xl bg-accent text-white text-sm font-semibold hover:bg-accent-light transition-colors">Save all</button>
        </div>
      )}
    </>
  );
